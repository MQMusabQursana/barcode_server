...
var onUserLocationChanged = async function (userId, lat, lng, angle,userJobState) {
  return new Promise(async function (resolve, reject) {
    try {
      const socketIO = require("./socket_server").getio();
      socketIO.in(SocketRooms.managers).emit(SocketEvents.onUserLocationChanged, { user_id: userId, lat: lat, lng: lng, angle: angle,user_job_state:userJobState });
      console.log("sent in SocketRooms.managers -> " + JSON.stringify({ user_id: userId, lat: lat, lng: lng, angle: angle,user_job_state:userJobState }));
